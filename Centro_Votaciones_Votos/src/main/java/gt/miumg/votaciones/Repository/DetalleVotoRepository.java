...
//consulta para traer los votos para alguna grafica
    @Query(value = "SELECT\n"
            ...
            + "    public.voto v\n"
            + "INNER JOIN\n"
            + "    public.detallevoto dv ON v.idvoto = dv.idvoto -- Unimos con la tabla de detalles de votos\n"
            + "INNER JOIN\n"
            + "    public.catalogo c ON CAST(dv.idelecto AS integer) = c.idcatalogo  -- Unimos con la tabla de candidatos\n"
            + "LEFT JOIN\n"
            + "    public.catalogo ca ON c.partidopolítico = ca.idcatalogo -- Unimos con la tabla de partidos políticos\n"
            + "LEFT JOIN\n"
            + "    public.catalogo mun ON c.municipio = mun.idcatalogo -- Unimos con la tabla de municipios a través del candidato\n"
            + "LEFT JOIN\n"
            + "    public.catalogo dep ON c.departamento = dep.idcatalogo -- Unimos con la tabla de departamentos a través del candidato\n"
            + "INNER JOIN\n"
            + "    public.catalogo cat ON c.tipocandidato  = cat.idcatalogo -- Unimos con la tabla de tipos de candidatos\n"
            + "WHERE\n"
